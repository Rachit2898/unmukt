...
import { View, Text,SafeAreaView, ScrollView, TouchableOpacity, Image,FlatList, Linking, Dimensions ,StyleSheet,Alert} from 'react-native'
...
const AcceptRawMaterial = ({navigation,route}) => {
    ...
    const handelSubmit =(item)=>{
        ...
        try{
            ...
            Axios.put(`infradailyrecords`,data)
                .then(res=>{
                    const index = pendingIncoming.findIndex(elm=>elm.id == res.data.id)
    ...
    const renderList =({item})=>{
        ...
        return(
            <TouchableOpacity onPress={()=>handelSubmit(item)} style={styles.listContainer}>
                <View style={styles.innercontaner}>
                    <Text style={styles.text2}>{tractor.modelName +' '+tractor.engineNumber}</Text>
                    <Text style={[styles.text2,{marginLeft:20,marginRight:20}]}>{item.looseMaterialIndicator == 'Y' ?  'Loose':` ${item.countOfBales}` }</Text>
                </View>
