...
const HarvestingSub = props => {
  ...
  useFocusEffect(
    useCallback(() => {
      Axios.get(
        `harvestingCollectionDetails/${farmerId}/${landDetails.id}/${collectionCycle}/H/Y`,
      )
        .then(Hres => {
      ...
      Axios.get(
        `getAllInfraDetailsbyType/H/${collectionCycle}/${
      ...
      {harvestingDetails?.processCode == 'E' && (
        <View
          ...
          <Text style={{color: '#39a029', fontSize: 18, fontWeight: 'bold'}}>
            HAEVESTING HAS BEEN COMPLETED
          </Text>
          <Text style={{color: '#a07229', fontSize: 14, fontWeight: 'bold'}}>
            Start date :{' '}
            {dateFormater(
              props?.completeharvesterdetails?.actualHarvestingStartDate,
            )}
          </Text>
          <Text style={{color: '#a07229', fontSize: 14, fontWeight: 'bold'}}>
            End date :{' '}
            {dateFormater(
              props?.completeharvesterdetails?.actualHarvestingEndDate,
            )}
          </Text>
        </View>
      )}
