...
const RawMaterialSub = (props) => {
    ...
    useFocusEffect(
        useCallback(()=>{
            Axios.get(`harvestingCollectionDetails/${farmerId}/${landDetails.id}/${collectionCycle}/C/Y`)
                .then(Hres=>{
    ...
    return (
        ...
        {harvestingDetails?.processCode == 'E' &&
            ...
            >
                <Text style={{color:'#39a029',fontSize:18,fontWeight:'bold'}}>COLLECTION HAS BEEN COMPLETED</Text>
                <Text style={{color:'#a07229',fontSize:14,fontWeight:'bold'}}>Start date : {dateFormater(props?.completeharvesterdetails?.rawMaterialCollectionStartDate)}</Text>
                <Text style={{color:'#a07229',fontSize:14,fontWeight:'bold'}}>End date : {dateFormater(props?.completeharvesterdetails?.rawMaterialCollectionEndDate)}</Text>
            </View>
        }
        </>
    )
